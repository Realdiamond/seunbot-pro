...
// Curated default watchlist — merged with whatever the backend returns so the
// list is never sparse/empty on first load ("add more").
const DEFAULT_WATCHLIST = [
  'AAPL', 'MSFT', 'GOOGL', 'AMZN', 'META', 'TSLA', 'NVDA', 'NFLX', 'AMD', 'INTC',
  'JNJ', 'V', 'PG', 'WMT', 'JPM', 'BAC', 'DIS', 'KO', 'PEP', 'XOM',
  'CVX', 'PFE', 'MRK', 'ABBV', 'COST', 'AVGO', 'ORCL', 'CRM', 'ADBE', 'QCOM'
];

// Curated US ETF universe. The backend has no US-ETF classification yet
// (assetType=ETF only covers NGX), so the section curates well-known US ETFs
// and pulls live data/predictions from the same UsPrediction endpoints.
const US_ETFS = [
...
const USStocksDashboard = ({ initialSymbol = null, viewMode: initialViewMode = null }) => {
  ...
  const activeTab = useMemo(() => {
    // If initialViewMode prop is passed, use it (for symbol-based routes)
    if (initialViewMode === 'analysis') return 'advanced';
    if (location.pathname === '/usstocks-analysis') return 'advanced';
    if (location.pathname === '/usstocks-setups') return 'weeklySetups';
    if (location.pathname === '/usstocks-etfs') return 'etfs';
    // Symbol-based route should show advanced analysis
    if (initialSymbol) return 'advanced';
    return 'overview';
  }, [location.pathname, initialViewMode, initialSymbol]);
  ...
  const loadData = async () => {
    ...
    try {
      ...
      // backend watchlist with the curated defaults so it's never empty.
      const savedWatchlist = localStorage.getItem('us_stocks_watchlist');
      let currentWatchlist = [];
      if (savedWatchlist) {
        try { currentWatchlist = JSON.parse(savedWatchlist) || []; } catch { currentWatchlist = []; }
      }
      if (!currentWatchlist.length) {
        currentWatchlist = Array.from(new Set([...(defaultWatchlist || []), ...DEFAULT_WATCHLIST]));
        localStorage.setItem('us_stocks_watchlist', JSON.stringify(currentWatchlist));
      }
  ...
  // In watchlist mode, guarantee a row for every watchlisted symbol even if it
  // isn't in the loaded stock list (otherwise the watchlist looks empty).
  const stocksForView = (() => {
    if (viewMode !== 'watchlist') return stocks;
    const present = new Set(stocks.map(s => s.symbol));
    const synthetic = watchlist
      .filter(sym => !present.has(sym))
      .map(sym => ({
        symbol: sym, name: sym, exchange: 'US', sector: 'US',
        price: 0, change: 0, changePercent: 0, volume: 0, high: 0, low: 0,
        imageUrl: null, isMock: false, isReadyForPrediction: true, sources: []
      }));
    return [...stocks, ...synthetic];
  })();

  // Filter and sort stocks
  const filteredStocks = stocksForView
    .filter(stock => {
    ...
    <div className="min-h-screen bg-gray-900 p-4 sm:p-6">
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex overflow-x-auto whitespace-nowrap gap-1 bg-gray-800 rounded-lg p-1 scrollbar-none scroll-smooth">
          ...
          <button
            onClick={() => navigate('/usstocks-etfs')}
            className={`flex-shrink-0 flex items-center gap-2 px-4 py-2 rounded-lg transition-colors ${
              activeTab === 'etfs' ? 'bg-purple-600 text-white' : 'text-gray-400 hover:text-white'
            }`}
          >
        ...
        {activeTab === 'etfs' && (() => {
          ...
          const etfRows = US_ETFS.map(({ symbol, name }) => {
            const live = bySymbol.get(symbol);
            return live
              ? { ...live, name, sector: 'ETF' }
              : { symbol, name, exchange: 'US', sector: 'ETF', price: 0, change: 0, changePercent: 0, volume: 0, high: 0, low: 0, imageUrl: null, isMock: false, sources: [] };
          });
          const available = etfRows.filter(e => e.price > 0).length;
          return (
            <div className="bg-gray-800 rounded-lg p-4 sm:p-6 space-y-4">
              <div className="flex items-center justify-between flex-wrap gap-3">
                <div className="flex items-center gap-2">
                  <Globe className="w-5 h-5 text-purple-400" />
                  <h2 className="text-lg sm:text-xl font-semibold text-white">US ETFs</h2>
                  <span className="px-2 py-0.5 bg-purple-500/20 text-purple-300 rounded text-xs">{available}/{US_ETFS.length} live</span>
                </div>
                <p className="text-xs text-gray-400 max-w-md">
                  Curated US ETFs with live SeunBot predictions. Click any ETF to open full analysis.
                </p>
              </div>
              <div className="overflow-x-auto">
                <table className="w-full">
                  <thead className="bg-gray-700">
                    <tr>
                      ...
                      <th className="px-4 py-3 text-left text-xs font-medium text-gray-300 uppercase tracking-wider hidden sm:table-cell">Name</th>
                      <th className="px-4 py-3 text-right text-xs font-medium text-gray-300 uppercase tracking-wider">Price</th>
                      <th className="px-4 py-3 text-right text-xs font-medium text-gray-300 uppercase tracking-wider">% Change</th>
                      ...
                      >
                        <td className="px-4 py-4 whitespace-nowrap">
                          <div className="flex items-center gap-2">
                            ...
                            {etf.price > 0
                              ? <span className="text-[10px] px-1 bg-green-500/20 text-green-400 rounded">Live</span>
                              : <span className="text-[10px] px-1 bg-gray-600/40 text-gray-400 rounded">No data</span>}
                          </div>
                        </td>
                        <td className="px-4 py-4 whitespace-nowrap hidden sm:table-cell">
                          <div className="text-sm text-gray-300">{etf.name}</div>
                        </td>
                        <td className="px-4 py-4 whitespace-nowrap text-right">
                          <div className="text-sm font-semibold text-white">{etf.price > 0 ? `$${etf.price.toFixed(2)}` : '—'}</div>
                        </td>
                        <td className="px-4 py-4 whitespace-nowrap text-right">
                          <div className={`flex items-center justify-end gap-1 text-sm font-semibold ${etf.changePercent >= 0 ? 'text-green-400' : 'text-red-400'}`}>
                            {etf.price > 0 ? (<>{etf.changePercent >= 0 ? <TrendingUp className="w-4 h-4" /> : <TrendingDown className="w-4 h-4" />}{etf.changePercent >= 0 ? '+' : ''}{etf.changePercent.toFixed(2)}%</>) : '—'}
                          </div>
                        </td>
                        <td className="px-4 py-4 whitespace-nowrap text-center">
                          <button
                            onClick={(e) => { e.stopPropagation(); handleStockClick(etf); }}
                            className="px-2.5 py-1 bg-purple-600 hover:bg-purple-700 rounded text-white text-xs transition-colors inline-flex items-center gap-1"
                          >
                            <Brain className="w-3 h-3" />
                            <span className="hidden sm:inline">Analyze</span>
                          </button>
              ...
              <p className="text-[11px] text-gray-500">
                Note: full US-ETF universe & dedicated ETF signals are pending backend support — see handoff note.
              </p>
            </div>
          );
        })()}
