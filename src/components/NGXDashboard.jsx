...
const NGXDashboard = ({ onSelectPair, initialSymbol = 'GTCO' }) => {
  ...
  const [watchlist, setWatchlist] = useState(() => {
    ...
    const stored = localStorage.getItem('ngx_watchlist')
    return stored ? JSON.parse(stored) : ['GTCO', 'DANGCEM', 'MTNN', 'ZENITHBANK']
  })
  ...
  const addToWatchlist = (symbol) => {
    if (!watchlist.includes(symbol)) {
      ...
      localStorage.setItem('ngx_watchlist', JSON.stringify(updated))
    }
  }
